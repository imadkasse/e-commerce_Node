...
const ShoppingCartBtnClient = ({ productId, isInShoppingCart }: Props) => {
  const token = Cookies.get("token");
  const [loading, setLoading] = useState<boolean>(false);
  const router = useRouter();
  const { user, setUser } = useUser();
  const [shopPro, setShopPro] = useState<boolean>(isInShoppingCart);
...
export default ShoppingCartBtnClient;
